...
def get_questions_options_from_params(params):
    question_ids = [o.get("question").id for p in params for o in p.get("options")]
    options = [b for p in params for o in p.get("options") for b in o.get("option")]
    return question_ids, options


def filter_by_criteria(
    params, question_ids, options, administration_ids, filter, is_map=False, data_ids=[]
):
    result = []
    ...
    data_views = data_views.values_list("data_id", "question_id", "options")

    df = pd.DataFrame(list(data_views), columns=["data_id", "question_id", "options"])
    for param in params:
        filter_criteria = []
        for index, option in enumerate(param.get("options")):
            ...
            if df.shape[0]:
                filter_df = df[
                    (df["question_id"] == question) & (df["options"].isin(opts))
                ]
                if filter_criteria:
                    filter_df = filter_df[(filter_df["data_id"].isin(filter_criteria))]
                ...
                if filter_df.shape[0]:
                    filter_df = filter_df[~filter_df["data_id"].isin(filter_criteria)]
                    filter_criteria += list(filter_df["data_id"].unique())
        if is_map:
            result.append(len(filter_criteria))
        if not is_map:
            result.append({"name": param.get("name"), "value": len(filter_criteria)})
    del df
    del question_ids
    del options
    return result
...
def get_advance_filter_data_ids(form_id, administration_id, options, is_glaas=False):
    ...
    if options:
        ...
        for qid in qids:
            ...
            data = data.filter(
                reduce(or_, [Q(options__contains=op) for op in or_filter_value])
            )
...
def transform_glass_answer(temp, questions, data_ids):
    for q in questions:
        value = None
        answers = Answers.objects.filter(question_id=q.id, data_id__in=data_ids)
        if q.type == QuestionTypes.option:
            options = QuestionOptions.objects.filter(question_id=q.id).values_list(
                "name", flat=True
            )
            value = {opt: 0 for opt in options}
            answers = answers.values("options").annotate(count=Count("options"))
            for a in answers:
                key = a.get("options")[0]
                prev = value.get(key)
        ...
        if q.type == QuestionTypes.number:
            value = 0
            answers = answers.values("value").annotate(sum=Sum("value"))
            for a in answers:
                value += a.get("sum")
