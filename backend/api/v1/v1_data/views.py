...
from api.v1.v1_forms.models import Forms, Questions, ViewJMPCriteria
...
class FormDataAddListView(APIView):
    ...
    def put(self, request, form_id, version):
        ...
        serializer = SubmitFormDataAnswerSerializer(data=request.data, many=True)
        if not serializer.is_valid():
            return Response(
                {
                    "message": validate_serializers_message(serializer.errors),
        ...
        if is_super_admin or is_county_admin_with_county_form:
            ...
            for answer in answers:
                ...
                if not form_answer:
                    form_answer = Answers(
                        data=data, question_id=answer.get("question"), created_by=user
                    )
        ...
        return Response(
            {"message": "store to pending data success"}, status=status.HTTP_200_OK
        )
...
class DataAnswerDetailDeleteView(APIView):
    ...
    def get(self, request, data_id, version):
        data = get_object_or_404(FormData, pk=data_id)
        return Response(
            ListDataAnswerSerializer(instance=data.data_answer.all(), many=True).data,
            status=status.HTTP_200_OK,
        )

    @extend_schema(
        responses={204: OpenApiResponse(description="Deletion with no response")},
        tags=["Data"],
...
def get_map_county_data_point(request, version, form_id):
    ...
    administrations = Administration.objects.filter(level_id=2).values("id", "name")
    counties = []
    form_data = instance.form_form_data
    if options_query:
        data_ids = get_advance_filter_data_ids(
    ...
    for adm in administrations:
        level3 = Administration.objects.filter(parent_id=adm.get("id")).values_list(
            "id", flat=True
        )
        childs = Administration.objects.filter(parent_id__in=list(level3)).values_list(
            "id", flat=True
        )
        filtered = filter(lambda d: d.get("administration_id") in list(childs), data)
        counties.append(
            {
                "loc": adm.get("name"),
...
def get_chart_data_point(request, version, form_id):
    ...
    if stack:
        ...
        for so in stack_options:
            query_set = (
                answers.filter(question=stack, options__contains=so.name)
                .values("options")
                .annotate(
                    ids=StringAgg(
            ...
            for val in query_set:
                ...
                if question.type == QuestionTypes.option:
                    child_query_set = child_query_set.values("options").annotate(
                        c=Count("options")
                    )
                    for child in child_query_set:
                        values.get("child").append(
                            {"name": child.get("options")[0], "value": child.get("c")}
                        )
                # Number type
                ...
                if question.type == QuestionTypes.multiple_option:
                    ...
                    for mo in multiple_options:
                        count = child_query_set.filter(
                            options__contains=mo.name
                        ).count()
                        values.get("child").append({"name": mo.name, "value": count})
            data.append(values)
            del values

        return Response({"type": "BARSTACK", "data": data}, status=status.HTTP_200_OK)
    ...
    for child in childs:
        ...
        query_set = (
            Answers.objects.filter(
                data_id__in=data_ids, question=serializer.validated_data.get("question")
            )
            .values("options")
            .annotate(
                c=Count("options"),
...
def get_last_update_data_point(request, version, form_id):
    form = get_object_or_404(Forms, pk=form_id)
    data = (
        FormData.objects.filter(form=form)
        .annotate(last_update=Coalesce("updated", "created"))
        .latest("last_update")
    )
    if not data:
        return Response(
            ...
            value=[
                {
                    "loc": "Baringo",
                    "data": {
                        "drinking water service level": {"basic": 10, "limited": 20}
                    },
                    "total": 30,
                }
            ],
...
def get_jmp_data(request, version, form_id):
    ...
    if options:
        is_jmp_criteria = jmp_criteria_obj.filter(
            reduce(or_, [Q(criteria__contains=op) for op in options])
        ).first()
    ...
    if request.GET.getlist("avg"):
        avgs = Questions.objects.filter(pk__in=request.GET.getlist("avg")).all()
    if request.GET.getlist("sum"):
        sums = Questions.objects.filter(pk__in=request.GET.getlist("sum")).all()
        for q in sums:
            if q.type in [QuestionTypes.option, QuestionTypes.multiple_option]:
                opts[q.id] = list(
                    q.question_question_options.order_by("order").values_list(
                        "name", flat=True
                    )
                )
    ...
    for adm in administration:
        ...
        for crt in criteria:
            ...
            if data_ids:
                ...
                if not matches:
                    matches = jmp_criteria_obj.filter(name=crt.name, level=crt.level)
                ...
                data = ViewJMPCount.objects.filter(
                    path__startswith=adm_path, name=crt.name, level=crt.level, form=form
                ).aggregate(Sum("total"))
...
def get_period_submission(request, version, form_id):
    ...
    if adm.path:
        adm_path = "{0}{1}.".format(adm.path, adm.id)
    fd = FormData.objects.filter(form=form, administration__path__startswith=adm_path)
    if data_ids:
    ...
    if not criteria:
        criteria = (
            ViewJMPCriteria.objects.filter(form=form).distinct("name", "level").all()
        )
        create_cache(criteria_cache, criteria)
            ...
            for crt in criteria:
                ...
                if not matches:
                    matches = ViewJMPCriteria.objects.filter(
                        form=form, name=crt.name, level=crt.level
                    ).count()
                    create_cache(matches_name, matches)
...
def get_glaas_data(request, version, form_id):
    ...
    for adm in administration:
        ...
        form_data_ids = form_datas.filter(**filter_form_data_counties).values_list(
            "id", flat=True
        )
