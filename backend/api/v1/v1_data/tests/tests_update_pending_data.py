...
class UpdatePendingDataTestCase(TestCase):
    def test_update_pending_data(self):
        ...
        data = self.client.get(
            "/api/v1/form-pending-data/{0}?lang=en".format(form_id),
            **{"HTTP_AUTHORIZATION": f"Bearer {token}"},
        )
        self.assertEqual(data.status_code, 200)
        self.assertEqual(
                ...
                "data": [
                    {
                        ...
                        "created": datetime.today().strftime("%B %d, %Y"),
                        "created_by": pending_form_data.created_by.get_full_name(),
                        "data_id": None,
                        "duration": 0,
                        "form": 1,
                        "geo": [6.2088, 106.8456],
        ...
        payload = [
            ...
            {"question": 104, "value": 4},
            {"question": 109, "value": 5.5},
        ]
        data = self.client.put(
            "/api/v1/form-pending-data/{0}?pending_data_id={1}".format(
                form_id, pending_data_id
            ),
            payload,
            content_type="application/json",
            **{"HTTP_AUTHORIZATION": f"Bearer {token}"},
        ...
        pending_form_data = PendingFormData.objects.filter(pk=pending_data_id).first()
        ...
        for d in data:
            ...
            if d["question"] == 104:
                self.assertEqual(isinstance(d["value"], int), True)
                self.assertEqual(d["value"], 4)
                self.assertEqual(isinstance(d["history"][0]["value"], int), True)
                self.assertEqual(d["history"][0]["value"], 2)
            if d["question"] == 109:
                self.assertEqual(isinstance(d["value"], float), True)
                self.assertEqual(d["value"], 5.5)
                self.assertEqual(isinstance(d["history"][0]["value"], float), True)
