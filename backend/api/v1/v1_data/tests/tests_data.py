...
class DataTestCase(TestCase):
    def test_list_form_data(self):
        ...
        data = self.client.get(
            "/api/v1/form-data/1?page=1&administration=1", follow=True, **header
        )
        result = data.json()
        self.assertEqual(data.status_code, 200)
        self.assertEqual(list(result), ["current", "total", "total_page", "data"])
        self.assertEqual(
            list(result["data"][0]),
            [
                "id",
        ...
        data = self.client.get("/api/v1/form-data/1?page=1", follow=True, **header)
        self.assertEqual(data.status_code, 200)
    ...
    def test_datapoint_deletion(self):
        ...
        res = self.client.delete(f"/api/v1/data/{data_id}", follow=True, **header)
    ...
    def test_datapoint_with_history_deletion(self):
        ...
        data = self.client.post(
            "/api/v1/form-data/1/", payload, content_type="application/json", **header
        )
        self.assertEqual(data.status_code, 200)
        data = data.json()
        ...
        res = self.client.get(
            f"/api/v1/data/{data_id}", content_type="application/json", **header
        )
        self.assertEqual(res.status_code, 200)
        res = res.json()
