...
@override_settings(
    ...
    LOGGING={
        "loggers": {
            "django_q": {
                "handlers": ["console"],
                # (CRITICAL to disable "[Q] Enqueued" messages)
                "level": "CRITICAL",
            },
        },
    },
...
class FormDataUpdateTestCase(TestCase):
    def test_update_datapoint_by_admin_role(self):
        ...
        for d in data:
            ...
            if question == 101:
                ...
                self.assertEqual(list(history[0]), ["value", "created", "created_by"])
    ...
    def test_update_datapoint_by_data_entry_role(self):
        ...
        res = self.client.post(
            "/api/v1/user", payload, content_type="application/json", **header
        )
        self.assertEqual(res.status_code, 200)
        self.assertEqual(res.json(), {"message": "User added successfully"})
        data_entry = SystemUser.objects.filter(email="wayan@example.com").first()
        # check invitation
        ...
        data_entry_user = {"email": "wayan@example.com", "password": "Test123*"}
        data_entry_user = self.client.post(
            "/api/v1/login", data_entry_user, content_type="application/json"
        )
        ...
        response = self.client.get(
            "/api/v1/profile", content_type="application/json", **header
        )
        ...
        data = self.client.get(
            "/api/v1/form-data/1?page=1",
            content_type="application/json",
            **{"HTTP_AUTHORIZATION": f"Bearer {data_entry_user_token}"},
        )
        self.assertEqual(data.status_code, 200)
        data = data.json()
        self.assertEqual(len(data["data"]) > 0, True)
        ...
        data = self.client.get(
            "/api/v1/form-pending-data/1?page=1",
            content_type="application/json",
            **{"HTTP_AUTHORIZATION": f"Bearer {data_entry_user_token}"},
        )
        self.assertEqual(data.status_code, 200)
        ...
        data = self.client.get(
            f"/api/v1/pending-data/{pending_data_id}?lang=en",
            content_type="application/json",
            **{"HTTP_AUTHORIZATION": f"Bearer {data_entry_user_token}"},
        )
        self.assertEqual(data.status_code, 200)
        ...
        self.assertEqual(len(data) > 0, True)
        results = [
            {"history": None, "question": 101, "value": "User Wayan"},
            {"history": None, "question": 102, "value": ["Male"]},
        ]
    ...
    def test_update_datapoint_by_county_admin_with_national_form(self):
        ...
        payload = {
            ...
            "answer": [
                {"question": 201, "value": "Made"},
                {"question": 202, "value": ["Other"]},
                {"question": 203, "value": 31208200175},
                {"question": 204, "value": 2},
                {"question": 205, "value": [6.2088, 106.8456]},
                {"question": 206, "value": ["Parent", "Children"]},
            ],
        }
        data = self.client.post(
            "/api/v1/form-data/2/",
        ...
        # county admin login
        county_admin = {"email": "county_admin@example.com", "password": "Test123*"}
        county_admin = self.client.post(
            "/api/v1/login", county_admin, content_type="application/json"
        )
        ...
        # Get all data from form
        data = self.client.get(
            "/api/v1/form-data/2?page=1",
            content_type="application/json",
            **{"HTTP_AUTHORIZATION": f"Bearer {county_admin_token}"},
        )
        self.assertEqual(data.status_code, 200)
        data = data.json()
        self.assertEqual(len(data["data"]) > 0, True)
        ...
        # Update data for question 101 and 102
        payload = [
            {"question": 201, "value": "Made County Admin"},
            {"question": 202, "value": ["Male"]},
        ]
        data = self.client.put(
            f"/api/v1/form-data/2?data_id={data_id}",
            payload,
            content_type="application/json",
            **{"HTTP_AUTHORIZATION": f"Bearer {county_admin_token}"},
        )
        ...
        # Get all data from pending data
        data = self.client.get(
            "/api/v1/form-pending-data/2?page=1",
            content_type="application/json",
            **{"HTTP_AUTHORIZATION": f"Bearer {county_admin_token}"},
        )
        self.assertEqual(data.status_code, 200)
        data = data.json()
        self.assertEqual(len(data["data"]) > 0, True)
        self.assertEqual(data["data"][0]["name"], "Testing Data National From Type")
        ...
        self.assertEqual(len(data) > 0, True)
        results = [
            {"history": None, "question": 201, "value": "Made County Admin"},
            {"history": None, "question": 202, "value": ["Male"]},
        ]
        self.assertEqual(data, results)
        # test get form data with pending data object inside
        data = self.client.get(
