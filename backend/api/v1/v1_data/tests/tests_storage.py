...
class StorageTestCase(TestCase):
    def test_if_storage_path_exists(self):
        self.assertTrue(os.path.exists(STORAGE_PATH), "Storage path not exists")

    def test_upload(self):
        filename = generate_file("test", hex=True)
        uploaded_file = storage.upload(file=filename, folder="test")
        self.assertTrue(
            os.path.exists(f"{STORAGE_PATH}/test/{filename}"), "File not exists"
        )
    ...
    def test_upload_with_custom_filename(self):
        ...
        self.assertEqual(uploaded_file, f"{STORAGE_PATH}/test/{custom_filename}")
