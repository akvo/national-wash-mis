...
urlpatterns = [
    ...
    re_path(r"^(?P<version>(v1))/export/form/(?P<form_id>[0-9]+)", export_form_data),
    re_path(r"^(?P<version>(v1))/maps/(?P<form_id>[0-9]+)", get_map_data_point),
    re_path(
        r"^(?P<version>(v1))/maps/county/(?P<form_id>[0-9]+)", get_map_county_data_point
    ),
    re_path(r"^(?P<version>(v1))/chart/data/(?P<form_id>[0-9]+)", get_chart_data_point),
    re_path(
        r"^(?P<version>(v1))/chart/administration/(?P<form_id>[0-9]+)",
        get_chart_administration,
    ),
    re_path(
        r"^(?P<version>(v1))/chart/criteria/(?P<form_id>[0-9]+)", get_chart_criteria
    ),
    re_path(
