...
def download_status(request, version, task_id):
    ...
    return Response(
        {"status": JobStatus.FieldStr.get(job.status)}, status=status.HTTP_200_OK
    )
...
def download_list(request, version):
    queryset = request.user.user_jobs.filter(type=JobTypes.download).order_by(
        "-created"
    )
...
def upload_excel(request, form_id, version):
    ...
    user_name = "{0}_{1}".format(request.user.first_name, request.user.last_name)
